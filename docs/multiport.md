...
| [Manhattan 8-Port HDMI KVM Switch](https://manhattanproducts.eu/products/manhattan-en-8-port-hdmi-kvm-switch-152785) | ✔ | PiKVM 4+ working out-of-the box, OTG connected to keyboard USB port. Hotkey switching. The only exception seem to be non-working keyboard/mouse on Raspberry PI 4, but I had the same issue with included OTG USB cable which reported "bad USB cable?" when connected. |
