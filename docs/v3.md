...
??? note "See the diagram"
    ...
    14. **CSI-2 interface** and **HDMI backpowering** jumper, see [Step 9 of the Basic Setup](#basic-setup). Open: diode will stop current from HDMI input (backpower will be fixed), closed: will allow current from HDMI device. 
