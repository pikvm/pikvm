...
* While the 4Plus includes a 5v USBC port for power, this is only to be used as a backup. If using this port only, this essentially turns your Plus into a Mini, the bottom board will only be powered while the top board will not thus disabling the top board which has the additional USB ports making the PiKVM expander, incompatible.
